...
//! slows, or fails an ingest. When audit is disabled (the dev/test default) the sink is a no-op —
//! no channel, no worker.
//!
//! Sift emits `sift.unknown_template` (notice) when a brand-new message template first appears.
//! Only the logical fields ride the event (actor / action / target=template_id / severity /
//! detail); no raw log line is shipped.
...
impl AuditEvent {
    ...
    /// `notice`-severity event (a notable observation: a new template appeared).
...
impl AuditSink {
    ...
    /// they never fail startup or affect the ingest path. Must be called from within a tokio
    ...
    /// worker the event is DROPPED (drop counter + warn). NEVER blocks or errors the ingest path.
...
mod tests {
    ...
    fn disabled_sink_is_noop_and_never_drops() {
        ...
        for _ in 0..1000 {
            sink.emit(AuditEvent::notice("sift.unknown_template", "syslog", "t_abc", "ok"));
        }
    ...
    async fn emit_never_blocks_when_sink_unreachable() {
        ...
        for _ in 0..(QUEUE_CAPACITY * 8) {
            sink.emit(AuditEvent::notice("sift.unknown_template", "u", "t_x", "v1"));
        }
        assert!(
            sink.dropped() > 0,
