...
//!
//! Filament emits `filament.trace.error` when an ingested trace contains an error span (sampled).
//! Only the logical fields ride an event (actor=root service / target=trace_id / detail=error
//! count) — never span payloads or attributes.
...
impl AuditEvent {
    ...
    /// `warning`-severity event (a trace that carries an error span).
...
mod tests {
    ...
    fn disabled_sink_is_noop_and_never_drops() {
        ...
        for _ in 0..1000 {
            sink.emit(AuditEvent::warning("filament.trace.error", "svc", "trace-1", "2"));
        }
    ...
    async fn emit_never_blocks_when_sink_unreachable() {
        ...
        for _ in 0..(QUEUE_CAPACITY * 8) {
            sink.emit(AuditEvent::warning("filament.trace.error", "svc", "t", "1"));
        }
        assert!(
            sink.dropped() > 0,
